...
UPDATE_REMINDER_SECONDS = 36 * 60 * 60
...
class UpdateCheckError(ValueError):
    """Raised when local or remote release metadata is invalid."""
...
def fetch_release(url: str = REMOTE_RELEASE_URL) -> dict[str, Any]:
    ...
    with urllib.request.urlopen(request, timeout=NETWORK_TIMEOUT_SECONDS) as response:
        ...
        if (
            ...
            or final_url.query != expected_url.query
        ):
...
def normalize_state(
    raw: dict[str, Any], local_release: dict[str, Any]
) -> dict[str, Any]:
    if (
    ...
    try:
        ...
        return initial_state(local_release)
    if not isinstance(raw.get("update_pending"), bool):
        return initial_state(local_release)
...
def build_notice(
    *,
    ...
    return {
        ...
        "reminder_interval_seconds": UPDATE_REMINDER_SECONDS,
...
def run_scheduler(
    *,
    ...
    state = normalize_state(raw_state, local_release)
    ...
    if parse_version(remote_release["version"]) <= parse_version(
    ...
    reminder_count = (
        int(state["reminder_count"]) + 1 if state.get("update_pending") else 1
    )
    next_state = schedule_state(
        state,
        local_version=local_release["version"],
        now=current_time,
        delay_seconds=UPDATE_REMINDER_SECONDS,
    )
    next_state.update(
        {
            "update_pending": True,
