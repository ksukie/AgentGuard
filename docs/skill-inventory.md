...
这是第四项功能唯一保留的公开示例，不是精确匹配口令。只要当前消息显式调用 AgentPolicy，并涉及发现、查找、检查、审计或排查本地/当前 Codex Skill，都会进入同一个运行时清单流程，不会拆成不同功能或改走文件缓存扫描。全量问题列出全部 Skill；针对某个 Skill 的问题则基于同一份完整清单聚焦回答。
